...
**EDITOR-2D2**, documented below, now owns the create writer (fresh re-read, fingerprint, DataHandler create, read-back verifier, PRG). This section describes the review-only 2D1 milestone.
...
## EDITOR-2D2 — Confirmed creation of one PriceOption

EDITOR-2D2 adds the sole confirmed create path under an existing Placement, extending the review-only 2D1 milestone above. Review remains read-only. `priceOptionCreateApply` uses its own `priceCreateApplyToken`, separate from `priceCreateToken`.

The single `arp-price-create-form` owns the live label/price fields, selectors, both tokens and `confirmedFingerprint`. READY's Save button targets that form; there are no shadow label/price fields. Editing either value after Review requires refreshed confirmation.

Apply validates CSRF, repeats the accepted PriceCreate permission preflight, reloads the graph and existing default-language options, and rebuilds the pure plan. Preparation blockers stop before the writer. A strict lowercase 64-hex SHA-256 `price-option-create-v1` fingerprint covers pid; Menu/Category/Placement/Item uid, UUID and tstamp; all existing options' uid, UUID, tstamp, normalized stored label, amount, sorting and hidden; and the requested normalized label, amountMinor and fresh plannedSorting. Existing options are sorted by uid before hashing. Display titles and formatted amounts are excluded.

Malformed confirmation stops before writing. A stale confirmation writes nothing, renders the prominent stale warning and refreshed READY/Save control. A concurrent duplicate instead renders the existing prominent blocker with no Save.

`RestaurantPriceOptionCreateWriter` alone invokes DataHandler. Its pure map contains exactly one PriceOption NEW row with `pid`, `placement`, `label`, `amount`, `sorting`, `sys_language_uid=0`. The deterministic temporary key is `NEWarpPriceCreate` plus 24 fingerprint characters. No parent row or existing PriceOption is written. UUID is generated by TYPO3 TCA; hidden is omitted and remains visible by default. No future uid, UUID or timestamps are minted.

The attempt flag is set immediately before `process_datamap()`. There is no command map, direct SQL mutation, transaction, rollback, retry or atomicity claim. Pre-attempt failures remain rendered. After any attempt, flash feedback and HTTP 303 redirect to `id`, `menu`, `priceOptionCreate=<placementUid>` (no fragment) reload the table and blank form.

Read-back uses the positive NEW substitution uid and exact PriceOption table mapping. It checks all six intended fields, uid, hidden=0, deleted=0, valid generated UUID v4 distinct from existing option UUIDs, and unchanged parent identity/tstamps and graph membership. `created` requires exact verification with no Core errors or process exception. A usable substitution with incomplete verification/diagnostics is `partialFailure`; missing/invalid substitution or pre-attempt failure is `failed`. Attempted failures never claim nothing was written. TYPO3 13/14 runtime acceptance remains a separate gate; no VPS or production deployment is part of this implementation.
