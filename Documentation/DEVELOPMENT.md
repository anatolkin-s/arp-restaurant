...
Bulk paste accepts TSV (Category, Item, Variant, Price), validates it in `BulkMenuParser`, and renders an editable in-request draft. `BulkDraftValidator` revalidates posted cell strings on `bulkDraftRevalidate`. `bulkDraftReset` rebuilds that draft from the last Preview TSV. Restore order is view-only. After a cell edit, the UI shows that server validation is stale until Revalidate. `bulkIdentityResolve` revalidates again, then runs read-only identity resolution (`RestaurantIdentityReader` + `BulkIdentityResolver`). `bulkApplyPrepare` revalidates and re-resolves again, then builds a read-only `ApplyPlan` / confirmation preview (`ApplyReady`). `bulkApply` rebuilds the plan, compares the confirmation fingerprint, and writes through DataHandler `process_datamap` only (append-only; PRG after any write attempt).

Existing PriceOption edit is a separate boundary: review via `priceOptionEditReview`, confirmed save via `priceOptionEditApply` (`RestaurantPriceOptionUpdateWriter`, label + amount only, fingerprint continuity, PRG after attempted write). PriceOption visibility is another separate boundary: review via `priceOptionVisibilityReview`, confirmed save via `priceOptionVisibilityApply` (`RestaurantPriceOptionVisibilityWriter`, `PriceOption.hidden` only). Effective frontend availability can still depend on Menu/Category/Placement/Item state and scheduling; this workflow does not override them. Adding a PriceOption under an existing Placement uses read-only review (`priceOptionCreateReview` / `priceCreateToken`) followed by **EDITOR-2D2 confirmed create** (`priceOptionCreateApply` / `priceCreateApplyToken`). Its dedicated writer creates exactly one PriceOption, with no parent writes; TYPO3 generates UUID v4 and the row is visible by default. Fresh graph/permission/plan validation and deterministic fingerprint comparison precede the writer. Stale confirmation writes nothing. HTTP 303 PRG follows only a DataHandler attempt. There is no transaction, rollback, retry or atomicity claim. Runtime acceptance on TYPO3 13/14 is a separate gate. See [EDITOR_WRITE_CONTRACT.md](EDITOR_WRITE_CONTRACT.md).
...
EDITOR-2B1.1 is **PASS / TYPO3 13 + TYPO3 14 LIVE ACCEPTED** at extension SHA `bd53dbe9acd08ac1df96113747027f0ed59502ae` on TYPO3 13.4.34 and TYPO3 14.3.6. That gate covers the editable temporary draft, Restore order vs Reset draft, Variant-run warnings, and Fluid `warnings` / `globalError` accessor fixes.

EDITOR-2B2 is **REPO IMPLEMENTED / READ-ONLY**. Resolve identities is an explicit CSRF POST after DraftValid. It does not Apply. See [EDITOR_WRITE_CONTRACT.md](EDITOR_WRITE_CONTRACT.md).
