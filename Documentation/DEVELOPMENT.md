...
Existing PriceOption edit is a separate boundary: review via `priceOptionEditReview`, confirmed save via `priceOptionEditApply` (`RestaurantPriceOptionUpdateWriter`, label + amount only, fingerprint continuity, PRG after attempted write). EDITOR-2C3 adds a **review-only** PriceOption visibility panel (`priceOptionVisibilityReview` / `priceVisibilityToken`) that owns `PriceOption.hidden` only. It does not persist, does not call DataHandler, and does not change parent hidden/scheduling. Effective frontend visibility can still depend on other records. See [EDITOR_WRITE_CONTRACT.md](EDITOR_WRITE_CONTRACT.md).
