...
Existing PriceOption edit is a separate boundary: review via `priceOptionEditReview`, confirmed save via `priceOptionEditApply` (`RestaurantPriceOptionUpdateWriter`, label + amount only, fingerprint continuity, PRG after attempted write). See [EDITOR_WRITE_CONTRACT.md](EDITOR_WRITE_CONTRACT.md).
