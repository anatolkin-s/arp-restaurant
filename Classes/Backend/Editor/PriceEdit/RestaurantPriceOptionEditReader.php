...
/**
 * SELECT-only loader for an existing PriceOption edit context.
 *
 ...
 */
final class RestaurantPriceOptionEditReader
{
    public function __construct(
        private readonly ConnectionPool $connectionPool,
        private readonly PriceOptionEditGraphAssessor $graphAssessor,
    ) {}
    ...
    public function load(
        ...
        if ($priceOptionUid <= 0 || $pid <= 0 || $selectedMenuUid <= 0) {
            return new PriceOptionEditLoadResult(
                outcome: 'blocked',
                context: null,
                blockers: [new PriceOptionEditBlocker('missingPriceOption')],
            );
        }
